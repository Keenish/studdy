...
### 기계 간 재현성 — 안 된다 (2026-07-31)

"이 맥에서만 기록·비교했다"로 오래 열려 있던 항목이다. CI를 걸어 **다른 기계에서 돌려 답을 받았다.**

| | 로컬 | GitHub Actions runner |
|---|---|---|
| macOS | 26.5.2 | 26.4 |
| Xcode | 26.6 | 26.5 |
| Swift | 6.3.3 | 6.3.2 |

**스냅샷 6건 전부 실패했다.** 나머지 검사 14개는 전부 통과했으므로 코드 문제가 아니다.

허용 오차로 살려 보려 했다 — `precision 0.99` / `perceptualPrecision 0.97`. 로컬에서는 정상 통과하면서 토큰을 깨뜨리면 3건이 잡히는, 원하는 성질을 유지했다. **그런데 CI에서는 여전히 6건 전부 실패했다.** 안티에일리어싱 수준의 차이가 아니라 **폰트 메트릭이 달라 레이아웃 자체가 어긋나는** 것이다. 오차로 메울 수 있는 종류가 아니다.

그래서 오차를 되돌렸다. 해결하지 못한 문제를 위해 가드를 약화시킬 이유가 없다 — 로컬에서는 완전 일치가 가장 엄격하다.

**결론: 스냅샷은 로컬 전용 가드다.** CI는 `SKIP_SNAPSHOTS=1`로 제외하고, **건너뛰었다는 사실을 로그에 찍는다.** 조용히 빼면 안 돈 검사를 통과로 읽게 된다 — 이 저장소가 가장 경계하는 실패다.

읽어낼 것: **스냅샷 테스트를 CI에 걸려면 렌더 환경을 고정해야 한다.** 컨테이너 이미지나 툴체인 핀 없이 "참조 이미지를 커밋한다"만으로는 팀에서 안 돈다. 이걸 모르고 CI에 넣으면 아무도 신뢰하지 않는 빨간 파이프라인이 하나 생긴다.
...
| ~~스냅샷 참조 이미지의 기계 간 재현성~~ | **해소 (2026-07-31) — 답은 "안 된다"** . CI runner(OS·Xcode·Swift 전부 다름)에서 6건 전부 실패. 허용 오차로도 안 됐다 ([§5](#기계-간-재현성--안-된다-2026-07-31)). 로컬 전용 가드로 확정 |
