...
### 2-2c. 잘못 분류돼 있던 것 — 지금 닫았다 (2026-07-31)

둘 다 "실행 환경 의존"으로 적혀 있었는데 이 환경에서 되는 것이었다.

- [x] **TSan 런타임 검증** → [Concurrency/phase1-concurrency.md](Concurrency/phase1-concurrency.md#컴파일-통과가-런타임-안전을-뜻하는가-2026-07-31). 보고 **0건**, 양성 대조로 검사기가 살아 있음을 확인. 단 TSan은 실행된 경로만 본다
- [x] **`.equatable()` 효과** → [Swift/phase1-swiftui-rendering.md](Swift/phase1-swiftui-rendering.md#equatable-이-실제로-재평가를-줄이는가-2026-07-31). **200회 → 0회.** 첫 실험 설계가 틀렸고(비캡처 클로저) 양성 대조가 그걸 드러냈다

### 2-3. 아직 열려 있는 것

**"왜 필요한가"를 같이 적는다.** 전부 같은 무게로 보이면 어느 것도 안 하게 된다 — 링크 수집함에 세운 규칙과 같은 이유다.

#### 결론이 뒤집힐 수 있는 것 (iOS 시뮬레이터 필요)

- [ ] **macOS↔iOS 이식성** — Phase 1a의 헤드라인 발견이 *"행 body는 0회 — 통념과 달랐다"*다. SwiftUI의 건너뛰기 판정이 플랫폼마다 다르면 **통념을 뒤집었다는 결론 자체가 macOS 전용 잡학**이 된다. 이 저장소에서 가장 크게 내세운 발견이라 우선순위가 가장 높다
- [ ] **Dynamic Type 스케일** — 문서가 "고쳤다"고 말하는데, macOS에서는 고친 코드와 안 고친 코드를 **구별할 방법이 없다는 걸 측정으로 확정**했다. `@ScaledMetric` + 높이·패딩 동시 스케일이 실제로 맞는지 아무도 모른다. 접근성 항목이라 틀리면 사용자에게 바로 간다
- [x] **스냅샷 기계 간 재현성** — **닫혔다 (2026-07-31). 답은 "안 된다"** — CI runner에서 6건 전부 실패했고 허용 오차로도 안 됐다. 폰트 메트릭 차이라 오차로 메울 종류가 아니다. 스냅샷은 **로컬 전용 가드**로 확정하고 CI에서 제외한다([근거](DesignSystem/phase3-design-system.md#기계-간-재현성--안-된다-2026-07-31))

#### 원리적으로 닫히지 않는 것

todo가 아니라 **거짓 확신 방지 장치**다. 닫을 대상이 아니라 계속 열려 있어야 의미가 있다.

- 팀 온보딩 비용 — 혼자 하는 학습으로는 측정 조건이 성립하지 않는다
- 오류 발견율 — 못 잡은 오류의 수를 모른다 ([§4 머리말](AI/phase-parallel-ai-verification.md#가장-중요한-한계를-먼저))

#### 목록에서 뺀 것 (2026-07-31)

값이 낮아 지웠다. 열린 항목이 많으면 목록이 신호를 잃는다.

| 뺀 항목 | 이유 |
|---|---|
| Instruments SwiftUI 템플릿 | `OSSignposter`로 우회했고 **두 측정이 일치**했다. 남은 건 도구 사용법이지 주장이 아니다 |
| Phase 0 §7 실기기 | 위 iOS 항목이 커버한다. 중복 |
| TCA `previewValue` | 에피소드 서술 한 줄. 어떤 판단에도 쓰이지 않는다 |
| 화면 수 대비 빌드 시간 선형성 | 결정은 이미 증분 **8배**로 났다. 선형 여부가 선택을 바꾸지 않는다 |
